...
- LLM视角下，重新审视强化学习的各种概念
  - 基础概念：状态s，动作a，奖励r，策略π，环境
  - 进阶概念：轨迹τ，折扣因子γ，收益G，价值V，Q函数，优势A
- 优势A的推导
  - 蒙特卡洛法
  - 蒙特卡洛估计GRPO中的优势A
- 强化学习的目标函数与求导
...
- 状态s：
  - 初始状态 $s_1$ ：prompt $x$
  - t时刻状态 $s_t$ ：prompt $x$ + response的前t个token $y_{≤t}$
...
- 奖励r：
  ...
  - 前面的token：奖励为0
- 策略π：
  - LLM生成第t个token的概率 $P(y_t | x, y_{≤t-1})$
...
> - 动作a：大脑发出的指令“将枪口移动到新坐标 $(x_2,y_2)$ 并开枪”（把移动和开枪视为一整个动作）
...
很多强化学习的教程会强调马尔可夫性，但对于LLM，我们反而应该**忘记**马尔可夫性。
- 马尔可夫性： $P(a_{t}|s_1, a_1, ... , s_{t-1}, a_{t-1}, s_{t})=P(a_{t}|s_t)$ ，即下一个动作的概率与历史无关。
- 但在LLM中，预测每个token的概率是 $P(y_{t}|x, y_{<t})$ ，即每个token是和历史有关的，因此不满足马尔科夫性
- 满足马尔可夫性、历史无关的 $P(y_{t}|y_{t-1})$ 是传统n-gram模型的做法，不是LLM

当然，也可以“强行”让LLM满足马尔可夫性：
- 把所有历史拼接作为一个状态，它也是满足马尔可夫性的。
- 在LLM中，$s_1$ 是prompt $x$ ，$a_t$ 是response的第t个token $y_t$ ，状态 $s_{t+1}=s_t + a_t = x + y_{≤t}$
- 这种定义至少在形式上满足“马尔可夫性”：$P(a_{t}|s_1, a_1, ... , s_{t-1}, a_{t-1}, s_{t})=P(a_{t}|s_t)$
- 虽然看起来有点耍赖
...
- $V^π(s)$ 和 s 处于序列的第几步无关，只和s与π的取值有关
- 同一个状态s，在不同的策略π下，价值 $V^π(s)$ 可能不同
- $s_t$ 的价值是从 $s_t$ 的**后续**奖励 $r_t$ 开始算的，而不是从到达s时产生的奖励 $r_{t-1}$ 开始算的

### 辨析：奖励r、收益G、价值V
- 奖励reward：第t步行动**实际产生**的**即时**“好处”，衡量**状态s+动作a**的好坏
- 收益gain：从第t步到终局**实际产生**的**累计**“好处”，衡量**轨迹τ**（或其中某个片段）的好坏
- 价值V：即从s到终局时，**预期会产生**的**累计**“好处”，衡量**状态s**的好坏
...
### Q和V的差异与联系：
- $Q^π(s,a)$ 衡量**状态s下动作a**的好坏，即状态s**明确选择动作a**后的预期收获（只是说在这一步选择动作a，后续动作还是按策略π采样）
- $V^π(s)$ 衡量**状态s**的好坏，等价于在状态s按策略π随机动作，并且将这些**动作的效果累加**后的预期收益
...
- 从贝尔曼方程可以看出，优势 $A^π(s,a)$ 的**物理意义**：在状态s下，明确选择动作a，比按策略π随机选择一个动作，产生的**额外**的预期收益
  - $A^π(s,a)>0$ ：a是个好动作
  - $A^π(s,a)<0$ ：a是个坏动作
  - $A^π(s,a)=0$ ：a不好不坏
- 数学性质：给定状态s，所有动作a的优势 $A^π(s,a)$ 之和为0
  - 因为A衡量的是一个动作相比于所有动作的收益“相对值”，动作有好有坏，有的相对收益为正，就一定有的相对收益为负（相对收益、额外收益，指的是同一回事）
...
- 则每个token的优势A，就是 $\frac{整个response的优势A}{序列长度}$
- 对应GRPO中每个token的优势A
...
> - 引入detach，是为了强行让 $π_θ(τ|s_1)$ 能计算梯度，后面会介绍
> - $\frac{π_θ(τ|s_1)}{π_θ(τ|s_1)_{.detach}}$ 类似于gumbel-softmax中 $\text{OneHot}(s) - s_{.detach} + s$ ，在计算图中不能省略
> - $\frac{π_θ(τ|s_1)}{π_θ(τ|s_1)_{.detach}}$ 本质上是一种**重要性采样**，后面会介绍。
...
绝大多数机器学习问题，都是参数θ在打分函数 $f(x)$ 上，而分布 $p(x)$ 不含参数  
- 这种目标函数可以直接求导：  
...
> - 打分函数 $f_θ(x) \Rightarrow π_θ(y|x)$ ：给定prompt x，生成指定response y 的概率
...
> - 分布 $p_θ(x) \Rightarrow p_θ(x,y)=D(x) \cdot π_θ(y|x)$ ：从数据集采样 prompt $x$ ，从LLM采样 response $y$
...
> - 带参数的分布 $p_θ(x) \Rightarrow p_θ(x'|z)$ ，噪声 $z$ 映射出的图像 $x'$ 不是唯一的，而是一个图像分布
...
对于参数在分布上的目标函数 $J_θ = \mathbb{E}_{x \sim p_θ(x)} [f(x)]$  
...
> 注：为什么不像gumbel-softmax一样用  $g_θ(x) = \left( 1 - p_θ(x) + p_θ(x)_{.detach} \right) \cdot f(x)$：
> - $\frac{p_θ(x)}{p_θ(x)_{.detach}}$ 有实际物理意义（重要性采样），而 $1 - p_θ(x) + p_θ(x)_{.detach}$ 没有
...
\nabla_θ J_θ 
&= \nabla_θ \mathbb{E}_{x \sim p_θ(x)} [ f(x)] \\
&= \nabla_θ \mathbb{E}_{x \sim q(x)} [ g_θ(x)] \\
...
> 注：如何理解这种对目标函数的变换  
> - $J_θ = \mathbb{E}_{x \sim p_θ(x)} [f(x)] = \mathbb{E}_{x \sim q(x)} [g_θ(x)]$ 不是仅对某个 $x$ 和 $θ$ 成立，而是对所有 $x$ 和 $θ$ 都成立，这样才会有 $\nabla_θ \mathbb{E}_{x \sim p_θ(x)} [f(x)] = \nabla_θ \mathbb{E}_{x \sim q(x)} [g_θ(x)]$
>   - 如果只在某个点相等，而其他点不相等，则无法保证导数也恰好相等。
>   - 例： $y=x$ 和 $y=x^2$ 在x=0处也相等，但显然导数不相等
> - 换句话说， $\mathbb{E}_{x \sim p_θ(x)} [f(x)]$ 和 $\mathbb{E}_{x \sim q(x)} [g_θ(x)]$ 是**同一个函数的两种不同表达式**，因此它们每个点的导数才相同。
> - 两个不同的函数表达式却处处相等（本质上对应同一个函数）并不神奇。
>   - 一个平凡的例子：一个函数和它的泰勒展开
>   - $f(x)=\sin x, g(x)=x-\frac{x^3}{3!}+\frac{x^5}{5!}-...$ ，两者形式不同，但对于所有 $x$ 都有 $f(x)=g(x)$
>   - 它们的导数也是形式不同，但处处相等：$\nabla f(x)=\cos x, \nabla g(x)=1-\frac{x^2}{2!}+\frac{x^4}{4!}-... , \nabla f(x)=\nabla g(x)$
...
> 该小节可以略过，最基础的GRPO公式不涉及此内容
> - 在带有KL散度的GRPO中，对KL散度求导会涉及这种情况
> - 但实践中通常使用不加KL散度的GRPO，后续的DAPO、GSPO等改进版也明确去掉了KL散度
...
> - logits： $\log π_θ(a_t|s_t)$ 实际是每个动作的logits，而神经网络给每个token的打分在softmax归一化之前，天然就是logits，省略一步softmax有利于数值稳定
...
## 符号约定：
  ...
  - $π_θ$ 是训练框架（例如fsdp）部署的LLM，$π_{θ.detach}$ 是推理框架（例如vllm）部署的LLM
...
- 以上是GRPO公式**最本质**的部分。完整的GRPO只是加上了一些trick：
  ...
  - min-clip操作：off-policy情况下为了防止 $π_{old}$ 和 $π_θ$ 差距过大，导致训练不稳定
  - KL散度：为了防止RL训练后的 $π_θ$ 和其最初始版本 $π_{ref}$ 偏离太远，导致灾难性遗忘
...
- on-policy版本的GRPO，目标函数是0吗？
  - $J(θ)$ 确实等于0，但梯度 $\nabla J(θ)≠0$ 
  - 因为 $A$ 是通过reward归一化得到的，如果不考虑 $π_{θ.detach}$ 和 $π_θ$ 的数值差异，$J(θ)=0$ 
  - 后面章节会详细介绍为什么梯度  $\nabla J(θ)≠0$
...
- 计算图的角度：虽然目标函数值是0，但计算图中有 $π_θ(y|x)$ 节点，保证它会有梯度
...
response空间无限的例子不容易理解，这里举一个有限的例子
  ...
  - 梯度的方向：加强选项C，削弱ABD
  - 参数更新后，会得到一个新策略 $π_θ^*$ ，用新策略采样20个新response $\{y'_1, ..., y'_{20}\}$
...
- 在某一时刻，用于训练的 $π_θ$ 和用于推断的 $π_{old}$ 参数相同（但部署框架不同）
  ...
  - 最初始 $π_θ = π_{old}$ ，但之后参数 $π_θ$ 更新了N轮，而 $π_{old}$ 一直保持不变，且两者相差越来越大
  - 第N轮更新完后，使用 $π_θ$ 参数重新部署用于rollout的 $π_{old}$
- 此时两者参数又相等了，开始下个 N 轮的训练

训练时为什么不把 M*N 条数据一次全用掉？
- 直观理解：推理框架（如vllm）比训练框架（如fsdp）显存占用少，如果rollout M\*N条数据能把显存跑满，则训练用M\*N条数据一定会爆显存

为什么不通过梯度累加，用M\*N条数据更新一次参数？
- 没有必要，更新N次 vs 更新1次，前者更新频率更高，训练更“快”（但方差也更大）
- 使用M条数据更新时，本身往往也用了梯度累加（即真实的batch size比M更小，现在的框架如verl可以自动分配实际batch size并自动累加达到M）
...
- `actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu`：在gpu上计算时实际的batch size（会通过梯度累加达到ppo_mini_batch_size）
...
注意事项：
- min-clip是“不对称”的clip：触发clip的条件是 $A>0, \frac{π_θ}{π_{old}} > 1+ε \ 或者 \ A<0, \frac{π_θ}{π_{old}} < 1-ε$ ，即对于好动作只触发上clip，对坏动作只触发下clip。而不是对于任何动作都clip到 $1-ε < \frac{π_θ}{π_{old}} < 1+ε$ 的范围。
...
- 但更新尽量谨慎，不要乐观地大步前进（高概率好动作），也不要悲观地大步后退（低概率坏动作），因为模型估计可能不准
  - 乐观/悲观指的是策略 $π$ 的大小（主观的），而不是优势A的正负（客观的）
...
- 从动力学角度：
  ...
  - min-clip是一种**负反馈**，限制极端情况提供了一种“阻尼”，能在一个稳定区间内往复调整
...
- $r$ 是最终response
- $V_θ(s_t = x+y_{<t})$ 是个神经网络，用于估计状态 $s_t = x+y_{<t}$ 的价值。
 
价值网络 $V_θ(x+y_{<t})$ 的形式，与LLM计算句子概率的形式类似： $π_θ(x+y_{<t})$ ，因此在实现上，可以和 $π_θ(x+y_{<t})$ 共用相同的LLM，只是在输出时换用不同的head。

> 以上只是PPO最核心的思想。更多细节就不展开了（例如价值网络 $V_θ$ 如何训练、用广义优势估计GAE计算A）
...
> verl中的DAPO：<https://verl.readthedocs.io/en/latest/perf/best_practices.html>
...
- 第i个序列，所有token的优势之和 $A_{i}$ 也是个随机变量，可以观测（蒙特卡洛估计）
...
- $Y_3 = \frac{1}{\bar{L}} \sum_{t=1}^{L_i} A_{it} = \frac{1}{\bar{L}} A_i$ （DAPO单个token优势）
- $Y_4 = \frac{1}{G} \sum_{i=1}^G Y_3 = \frac{1}{G} \sum_{i=1}^G \frac{1} {\bar{L}} A_i=\frac{1}{token总数}\sum_{i=1}^G A_i$ （DAPO目标函数）

以及估计隐变量 $A_{it}$ 期望的几种不同口径（期望不可观测）：
- 单个序列内： $μ_{第i桶内} = \mathbb{E}_{序列内}[A_{it}]$，$i$ 固定，$t=1...L_i$
- 全量统计： $μ_{全量} = \mathbb{E}_{全部token}[A_{it}]$，$i=1...G$ ，$t=1...L_i$
- 分桶统计（每个序列为一桶）：$μ_{分桶} = \mathbb{E}_{全部序列}[ \mathbb{E}_{序列内}[A_{it}] ]$，$i=1...G$ ，$t=1...L_i$

则它们在统计上的对应关系：

| 统计量\期望 |  $μ_{第i桶内}$ | $μ_{全量}$ | $μ_{分桶}$ |
|--------|:-----------------------------:|:-----------------------------------------:|:-----------------------------------------:|
| $Y_1$  | **无偏**                      | 有偏<br>（无偏条件：所有 $μ_{第i桶内}$ 相等） | 有偏<br>（无偏条件：所有 $μ_{第i桶内}$ 相等） |
| $Y_2$  | 有偏<br>（无偏条件：$G=1$ 或所有 $μ_{第i桶内}$ 相等） | 有偏<br>（无偏条件：所有 $L_i$ 相等且所有 $μ_{第i桶内}$ 相等） | **无偏** |
| $Y_3$  | 有偏<br>（无偏条件：所有 $L_i = \bar{L}$） | 有偏<br>（无偏条件：所有 $L_i$ 相等且所有 $μ_{第i桶内}$ 相等） | 有偏<br>（无偏条件：所有 $L_i = \bar{L}$ 且所有 $μ_{第i桶内}$ 相等） |
| $Y_4$  | 有偏<br>（无实际无偏条件）    | **无偏**                                      | 有偏<br>（无偏条件：所有 $μ_{第i桶内}$ 相等） |

由此理解GRPO,DAPO,GSPO：
- GRPO：用统计量 $Y_2$ 估计期望 $μ_{分桶}$ （假设：每个序列贡献相等）
- DAPO：用统计量 $Y_4$ 估计期望 $μ_{全量}$ （假设：每个token贡献相等，等价于用token数量对每个序列的贡献加权）
- GSPO：不考虑隐变量 $A_{ij}$ ，直接使用可观测的 $A_i$

> 注：
> - 虽然DAPO用 $Y_3$ 估计 $μ_{第i桶内}$ 有偏，但各个序列的偏差能互相抵消，于是最后用 $Y_4$ 估计 $μ_{全量}$ 是无偏的。
...
则它们有如下性质（推导略过）
- $k_1$： 
  - 正向： $\mathbb{E}_{π_{θ.detach}}[k_1] = \text{KL}(π_θ | π_{ref})$
  - 反向： $\nabla_θ \mathbb{E}_{π_{θ.detach}}[k_1] = 0$
  - 性质：方差大，正向无偏，反向没有导数
- $k_2$：
  - 正向： $\mathbb{E}_{π_{θ.detach}}[k_2] ≈ \text{KL}(π_θ | π_{ref})$ （有偏，但偏差很小）
  - 反向： $\nabla_θ \mathbb{E}_{π_{θ.detach}}[k_2] = \nabla_θ \text{KL}(π_θ | π_{ref})$
  - 性质：方差小，正向有偏（但偏差很小，近似相等），反向无偏
- $k_3$：
  - 正向： $\mathbb{E}_{π_{θ.detach}}[k_3] = \text{KL}(π_θ | π_{ref})$
...
- $\frac{π_{θ}}{π_{θ.detach}}  k_3$：
  - 正向： $\mathbb{E}_{π_{θ.detach}}[\frac{π_{θ}}{π_{θ.detach}}  k_3] = \text{KL}(π_θ | π_{ref})$
